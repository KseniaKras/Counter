...
export const setNewMinCounterValue = (minValue: number) => ({type: SET_NEW_MIN_COUNTER_VALUE, minValue} as const)
export const setNewMaxCounterValue = (maxValue: number) => ({type: SET_NEW_MAX_COUNTER_VALUE, maxValue} as const)
export const setError = (error: boolean) => ({type: SET_ERROR, error} as const)
export const setCounterSettings = (counterSettings: boolean) => ({type: SET_COUNTER_SETTINGS, counterSettings} as const)
